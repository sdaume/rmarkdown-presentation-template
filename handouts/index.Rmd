---
title: "RMarkdown presentation handout template "
author: "The Speaker"
date: '`r Sys.Date()`'
output:
  tufte::tufte_html: 
...
link-citations: yes
subtitle: An implementation in R Markdown based on a package by JJ Allaire and Yihui Xie
...
```{r fig-margin-slide-1, out.width = "100%", fig.cap="These are handouts for a presentation held by __The Author__ on 01.03.2018 at _SOME PLACE_ on _SOME OCCASION_. The presentation is available in [HTML](https://sdaume.github.io/rmarkdown-presentation-template) and [PDF](https://github.com/sdaume/rmarkdown-presentation-template/pres.pdf); these handouts are also available as [HTML](https://sdaume.github.io/rmarkdown-presentation-template/handouts) and [PDF](https://github.com/sdaume/rmarkdown-presentation-template/handouts/pres_handout.pdf). The source for both is available in this [GitHub repository](https://github.com/sdaume/rmarkdown-presentation-template).", echo = FALSE}
knitr::include_graphics('./screenshots/rmd_pres_template_3_1280x720.png')
```


# About this handout template

## Background
This document is intended as a template for presentation handouts based on **RMarkdown** and can be used together with the  [reveal.js](https://revealjs.com/#/) RMarkdown presentation template^[See Github repository [rmarkdown-presentation-template](https://github.com/sdaume/rmarkdown-presentation-template)]. It is thus based on the same tools and workflows as the presentation template and makes it easy to use the same scripts when including for example charts, tables or other data generated with the help of R code chunks.

This handout template uses the Tufte handout style provided by the [**tufte** package](https://github.com/rstudio/tufte). The advantage of this approach is that handouts can be rendered both as HTML and PDF. 


## General editing options
For reference the original example document provided by JJ Allaire and Yihui Xie is included below. It covers all elements that might be used in this document and gives examples for positioning figures, creating tables and much more.


## Alternative styles
The default [tufte-css](http://rstudio.github.io/tufte/) style comes with a yellow-tinted background and otherwise follows the conventions in Tufte's books, for example using serif fonts. The variant used here is based on the Tufte style in HTML/CSS named "[Envisoned CSS](http://nogginfuel.com/envisioned-css/)". To use this style the YAML specifies:
...
If the `tufte_variant` is set to `default` knitr will render the document to the default Tufte style.

More variations of the traditional Tufte style are available. A style very similar to  *envisioned-css* provided with the `tufte` package is [`tint`](https://cran.r-project.org/web//packages/tint/vignettes/tintHTML.html). 

Finally, the original [Envisoned CSS](http://nogginfuel.com/envisioned-css/) provides a few more variations (table of contents that can be toggled^[A table of content can of course be included in the handouts by specifying `toc: true` in the YAML.], different colors etc) that are not provided through its implementation in the `tufte` package. In order to use this the generated HTML document will need to provide references to some additional CSS stylesheets and Javascript libraries.  

```{r fig-margin-slide-3, fig.margin = TRUE, fig.cap="These are handouts for a presentation held by __The Author__ on 01.03.2018 at _SOME PLACE_ on _SOME OCCASION_.", echo = FALSE}
knitr::include_graphics('./screenshots/rmd_pres_template_3_1280x720.png')
```

## Linking to your presentation

```{marginfigure}
The presentation is available in [HTML](https://sdaume.github.io/rmarkdown-presentation-template) and [PDF](https://github.com/sdaume/rmarkdown-presentation-template/pres.pdf); these handouts are also available as [HTML](https://sdaume.github.io/rmarkdown-presentation-template/handouts) and [PDF](https://github.com/sdaume/rmarkdown-presentation-template/handouts/pres_handout.pdf). The source for both is available in this [GitHub repository](https://github.com/sdaume/rmarkdown-presentation-template).
```

If you created a PDF version of your presentation with [DeckTape](https://github.com/astefanutti/decktape) and chose to generate screenshots of the same slides as well, it might be useful to include some key slides as reference points in the margins of this handout. 

As an example the title slide of the presentation template was included at the beginning of this document in the main body of text and a caption in the margins. Alternatively the slide image could have been placed in the margins as well with some margin notes to provide the links to the different versions of the presentation as well as the presentation source where applicable. 



## Credits
Similar to the presentation itself a handout should end with credits to the used tools. See the  instructions below if you want to add bibliography in addition to the references added by default in the margins.

The remainder of this document is taken from the sample handout detailing the tufte style^[See the live version at [http://rstudio.github.io/tufte/](http://rstudio.github.io/tufte/); the source can be found in [this GitHub repository](https://github.com/rstudio/tufte).]
